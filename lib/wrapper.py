...
def sharpness_fit_perturb_mcmc(xobso, yobso, edges_obs, tck_obs, tp_obs,
    ...
    ndim, nwalkers, nburn, nsteps = 2, 200, 200, 200
    ...
    def model(theta):#, obj_obs, xpdv, ypdv):
        ...
        hdist, _, _ = distance_to_edge(xdata, ydata, xedge_pdv, yedge_pdv, tck_pdv, tp_pdv, diagram=diagram, distance=distance)
        hdist = hdist + theta[0]
        ...
        return histy, xdata, ydata, number_reduction_factor

    def lnlikelihood(theta):#, obj_obs, xpdv, ypdv):
        histy, _, _, _ = model(theta)#, obj_obs, xpdv, ypdv)
    ...
    yfit, xdata, ydata, number_reduction_factor = model(para_fit)#, obj_obs, xpdv, ypdv)
    idx = reduce_samples(xdata.shape[0], xdata.shape[0]*number_reduction_factor)
    xdata, ydata = xdata[idx], ydata[idx]
    xfit = obj_obs.histx
    axes[0].step(xfit, yfit, **{"color":"blue", "label":"Galaxia best fitted model"})
