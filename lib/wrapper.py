...
def simulation(imc, Ndata, xp, yp, scalar, xpdv, ypdv, xedge_pdv, yedge_pdv, tck_pdv, diagram, distance, hist_model, obj_obs, e_xobs, e_yobs):
    # print(imc, "/", montecarlo)
...
def sharpness_fit_perturb(xobso, yobso, zobso, e_xobso, e_yobso, edges_obs, tck_obs, 
    ...
    for iz in range(Nz):
        ...
        for ix in range(xperturb.shape[0]):
            for iy in range(yperturb.shape[0]):
                ...
                obj.plot_hist(ax=axes[0], scale=Ndata/Nobs, histkwargs={"color":"blue", "label":"Galaxia"})
                obj.plot_fit(ax=axes[0], scale=Ndata/Nobs, fitkwargs={"color":"black", "label":"Galaxia fit"})
...
def sharpness_fit_rescale(xobso, yobso, zobso, e_xobso, e_yobso, edges_obs, tck_obs, 
        xpdvo, ypdvo, zpdvo, edges_pdv, tck_pdv,
        diagram, distance, hist_model,
