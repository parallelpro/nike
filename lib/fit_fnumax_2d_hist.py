...
Npdv = xpdv.shape[0]
mass = synp["mact"][idx]
g, teff = synp["mact"][idx]/synp["radius"][idx]**2.0, 10.0**synp["log_teff"][idx]/5777.0
...
filepath = "sample/test_2d_hist_nike_g/"
...
filepath = "sample/test_2d_hist_nike_g_teff/"
xobs, yobs = np.asarray(xobs), np.asarray(yobs)
xpdv, ypdv = np.asarray(xpdv), np.asarray(ypdv)
mass = np.asarray(mass)
...
# plt.savefig("sample/test/fit.png")
# plt.close()
