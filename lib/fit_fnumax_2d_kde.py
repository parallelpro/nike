...
Test if kde will improve the fit.
...
class fit_2d:
    def __init__(self, xobs, yobs, xmod, ymod, tck, 
        ...
        self.bins = [np.arange(20, 100, 0.5), np.arange(2.8, 4.0, 0.01)]
         # self.hobs, _, _ = np.histogram2d(self.xobs, self.yobs, bins=self.bins)

        self.Xgrid, self.Ygrid = np.mgrid[20:100:0.5, 2.8:4.0:0.01]
        self.Zobs = self._kde(self.Xgrid, self.Ygrid, self.xobs, self.yobs)

        return

    def _kde(self, Xgrid, Ygrid, xdata, ydata):
        positions = np.vstack([Xgrid.ravel(), Ygrid.ravel()])
        kernel = scipy.stats.gaussian_kde(np.vstack([xdata, ydata]), bw_method=0.1)
        Zdata = np.reshape(kernel(positions).T, Xgrid.shape)
        return Zdata
    ...
    def _plot_fit_results(self, para_fit):
        ...
        Zmod = self._kde(self.Xgrid, self.Ygrid, txmod, tymod)

        fig = plt.figure(figsize=(12, 18))
        axes = fig.subplots(nrows=3, ncols=2, squeeze=False).reshape(-1)
        ...
        ## plot kde
        extent = [self.Xgrid.min(), self.Xgrid.max(), self.Ygrid.min(), self.Ygrid.max()]
        # axes[4].imshow(np.rot90(self.Zobs), cmap=plt.cm.gist_earth_r, extent=extent)
        # axes[4].plot(self.xedge, self.yedge, "w-", linewidth=2, zorder=10) 
        # axes[5].imshow(np.rot90(Zmod), cmap=plt.cm.gist_earth_r, extent=extent)
        # axes[5].plot(self.xedge, self.yedge, "w-", linewidth=2, zorder=10) 

        axes[4].axis(extent)
        axes[5].axis(extent)

        axes[4].contourf(self.Xgrid[:,0], self.Ygrid[0,:], self.Zobs.T)
        axes[4].plot(self.xedge, self.yedge, "w-", linewidth=2, zorder=10) 
        axes[5].contourf(self.Xgrid[:,0], self.Ygrid[0,:], Zmod.T)
        axes[5].plot(self.xedge, self.yedge, "w-", linewidth=2, zorder=10) 
    ...
    def lnlikelihood(self, theta):
        ...
        ### method 2: 2d distance
        # hmod, _, _ = np.histogram2d(self.xmod*fnumax, self.ymod*(fnumax**0.75)/fdnu, bins=self.bins)
        # metric = np.sum(hmod*self.hobs)


        # # change to kde 2d
        Zmod = self._kde(self.Xgrid, self.Ygrid, self.xmod*fnumax, self.ymod*(fnumax**0.75)/fdnu)
        
        metric = np.sum(self.Zobs * Zmod)
...
filepath = "sample/test_2d_kde/"
xobs, yobs = np.asarray(xobs), np.asarray(yobs)
xpdv, ypdv = np.asarray(xpdv), np.asarray(ypdv)
mass = np.asarray(mass)

obj = fit_2d(xobs, yobs, xpdv, ypdv, tck,
     fix_fdnu=True, fix_fnumax=False, fdnu_value=[g, teff], fnumax_value=[g, teff])

para_priors = [[0., 10.], [-10., 10.], [-10., 10.]]
obj.run(para_priors, nburn=1000, nsteps=2000)
obj.output(filepath)

# para_fit = np.array([0.988, 0.04])
# fig = obj._plot_fit_results(para_fit)
# plt.savefig("sample/test_2d/fit.png")
# plt.close()
