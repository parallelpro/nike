...
from lib.wrapper import rgb_fit, rgb_combo_fit
import os
...
def loop(params):
    ...
    obsdir = rootpath+"sample/rgb/yu_nc/"
    moddir = rootpath+"sample/rgb/padova_nc/"
    ...
    # read in edges
    if var == 'numax':
        bump_obs_combo = np.load(obsdir+"numax_bump.npy", allow_pickle=True).tolist()
        bump_obs_combo = [b[0] for b in bump_obs_combo]
    ...
    if var == 'dnu':
        bump_obs_combo = np.load(obsdir+"dnu_bump.npy", allow_pickle=True).tolist()
        bump_obs_combo = [b[0] for b in bump_obs_combo]
    ...
    if var in ['mass', 'radius']:
        bump_obs_combo = np.load(obsdir+"mr_perturb_data.npy", allow_pickle=True).tolist()
        bump_obs_combo = [b[0] for b in bump_obs_combo]
    ...
    if j==0:
        # trial 1: upper limit
        filepath = rootpath+"sample/rgb/sharpness/kb95combo/"+var+"/ulim/"
        if not os.path.exists(filepath): os.mkdir(filepath)
        rgb_combo_fit(xobs, yobs, bump_obs_combo, xpdv, ypdv, bump_pdv,
            var, distance, filepath)
    else:
        # trial 2: lower limit
        filepath = rootpath+"sample/rgb/sharpness/kb95combo/"+var+"/llim/"
        if not os.path.exists(filepath): os.mkdir(filepath)
