...
mass_limits = [0.0, 1.3, 1.7, 3.0]

for imass in range(len(mass_limits)-1):
    mass = "{:0.0f}".format(imass)
    ...
    xobs, yobs, mass_obs = tnu_samples_obs[:,0], tnu_samples_obs[:,1], tnu_samples_obs[:,3]
    idx = (xobs**0.75/yobs>=ycut) & (yobs >= np.min(yedge_obs)) #& (yobs<=3.5)
    idx = idx & (mass_obs >= mass_limits[imass]) & (mass_obs < mass_limits[imass+1])
    xobs, yobs, mass_obs = xobs[idx], yobs[idx], mass_obs[idx]
    ...
    numax, dnu, mass_pdv = tnu_samples_pdv[:,0], tnu_samples_pdv[:,1], tnu_samples_pdv[:,3]
    idx = (numax**0.75/dnu>=ycut) & (dnu >= np.min(yedge_pdv)) #& (numax**0.75/dnu<=3.5)
    idx = idx & (mass_pdv >= mass_limits[imass]) & (mass_pdv < mass_limits[imass+1])
    dnu, numax, mass_pdv = dnu[idx], numax[idx], mass_pdv[idx]



    # # # customization
    # step 1: deal with numax
    filepath = rootpath+"sample/sharpness/perturb_gif_tnu/numax_mass/numax_"+mass+"/"
    if not os.path.exists(filepath): os.mkdir(filepath)
    output = filepath+"data_numax_mass_"+mass
    montecarlo = 120
    numax_perturb = np.arange(0.00, 0.050, 0.001) # np.arange(0.00, 0.06, 0.11)#
