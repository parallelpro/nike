...
def wrapper(filepath):
    # read data
    data = np.load(filepath+"data_mass.npy").tolist()
    mass_scatter = data["mass_perturb"]
    radius_scatter = data["radius_perturb"][0]
    ...
    x = mass_scatter*100
    y = slope-slope_obs
    axes[0].errorbar(mass_scatter*100, slope-slope_obs, yerr=eslope, fmt="k.", **errorbarkwargs)
    ...
    axes[0].axvline(mass_scatter[idx][0]*100, c="r", ls="--")
    axes[0].set_title("Scatter in radius: {:0.2f}%, fitted scatter in mass: {:0.2f}%".format(radius_scatter*100, mass_scatter[idx][0]*100))

    axes[0].set_xlabel("Scatter in mass relation [%]")
