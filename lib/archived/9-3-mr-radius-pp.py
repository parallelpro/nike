...
def wrapper(filepath):
    # read data
    data = np.load(filepath+"data_radius.npy").tolist()
    mass_scatter = data["mass_perturb"][0]
    radius_scatter = data["radius_perturb"]
    ...
    axes[0].axvline(radius_scatter[idx][0]*100, c="r", ls="--")
    axes[0].set_title("Fitted scatter in radius: {:0.2f}%, scatter in mass: {:0.2f}%".format(radius_scatter[idx][0]*100, mass_scatter*100))

    axes[0].set_xlabel("Scatter in radius relation [%]")
...
# 1 - radius - horizontal
filepath = rootpath+"sample/sharpness/perturb_gif_mr/radius/"
wrapper(filepath)
