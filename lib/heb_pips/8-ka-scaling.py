...
def loop(params):
    ...
    obsdir = rootpath+"sample/heb/ka_nc/"
    ...
    if var in ['mass', 'radius']:
        ...
        if distance == 'horizontal': # mass
            # to exclude those points which lies below the edge (so no horizontal distance).
            idx = (obs["radius_nc"] <= np.max(edges_obs[:,1])) & (obs["mass_nc"]<=1.25)
            for key in obs.keys():
                obs[key] = obs[key][idx]   
    ...
    if j==0:
        # trial 1: lower limit
        filepath = rootpath+"sample/heb/sharpness/ka18kb95/"+var+"/ulim/"   
        ...
        # trial 2: lower limit
        filepath = rootpath+"sample/heb/sharpness/ka18kb95/"+var+"/llim/"
        if not os.path.exists(filepath): os.mkdir(filepath)
